var viewRunTask = function () {
    ...
    var setDefaultValue = function () {
        objRun.taskCode = getUrlParam('taskCode');
        objRun.criteriaSetCode = getUrlParam('criteriaSetCode');
    ...
    var runTaskComplete = function (response) {
        ...
        if (response) {
            ...
            $("#runResult").html("计算任务成功完成。" + tempStr);
        } else {
            $("#runResult").html("计算任务执行失败");
        }
    }
    ...
    this.createSession = function () {
        ...
        setDefaultValue();
        webProxy.getTaskSessionContextByTaskCode(objRun.appDomain, objRun.taskCode, function (response) {
            var sContext = {
                appDomain: objRun.appDomain,
                sessionVariables: dataProcess.getRunTaskSessionVariables(dataProcess.variableXmlToJson(response), objRun.taskCode, objRun.criteriaSetCode),
            ...
            webProxy.createSessionByTaskCode(sContext, function (response) {
                ...
                objRun.sessionId = sessionID;
                getSessionProcessStatusList(objRun.appDomain, sessionID, loadSessionProcessStatusBegin);
