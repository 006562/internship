define('viewSearch', {
    ...
    contextMenu: function(obj) {
        ...
        $.anyContext({
            el: '#search-res',
            menu: [{
                text: self.$root.language == 'cn' ? '新建任务' : 'New TaskCode',
                callback: function (event) {
                    ...
                    if (!$(this).hasClass('disable')) {
                        var obj = {
                            ...
                            CodeDictionaryId: "",
                            CriteriaSetCode: "NewCriteriaCode" + that.taskCode,
                            IsCheckOut: "0",
                            IsCheck: true
                        };
                        ...
                        self.$root.$refs.task.operator = config.status.e_new;
                        self.$root.$refs.caculation.operator = config.status.e_new;
                        ...
                        self.$root.$refs.caculation.ecCode = obj.CriteriaSetCode;
                        self.$root.$refs.caculation.refreshCode = "notExistsCode";
                        self.$root.$refs.caculation.refreshIndex += 1;
                ...
                id: 'search-menu-copy',
                text: self.$root.language == 'cn' ? '复制' : 'Copy',
                disable: true,
                callback: function (event) {
                    event.stopPropagation();
                    if (!$(this).hasClass('disable')) {
                ...
                text: self.$root.language == 'cn' ? '粘贴' : 'Paste',
                disable: false,
                callback: function (event) {
                    event.stopPropagation();
                    if (!$(this).hasClass('disable')) {
                        var obj = {
                            ...
                            CodeDictionaryId: "",
                            CriteriaSetCode: self.$root.copyTaskModel[0].criteriaCode + "_Copy",
                            IsCheckOut: "0",
                            IsCheck: true
                        };
                        ...
                        self.$root.$refs.caculation.ecCode = obj.CriteriaSetCode;
                        self.$root.$refs.caculation.refreshCode = self.$root.copyTaskModel[0].criteriaCode;
                        self.$root.$refs.caculation.refreshIndex += 1;
                ...
                text: self.$root.language == 'cn' ? '删除' : 'Delete',
                disable: true,
                callback: function (event) {
                    event.stopPropagation();
                    if (!$(this).hasClass('disable')) {
                ...
                text: self.$root.language == 'cn' ? '生成脚本(T-SQL)' : 'script',
                disable: true,
                callback: function () {
                    if (!$(this).hasClass('disable')) {
                        var checkList = $.grep(self.searchResult, function (value, key) {
                        ...
                        var criteriaCode = checkList[0].CriteriaSetCode;
                        var appDomain = self.$root.appDomain;

                        dataProcess.scriptOfTask(appDomain, strTaskScript, taskCode, criteriaCode, scriptTemplates, function (strScript) {
        ...
        var search = Vue.extend({
            ...
            methods: {
                ...
                loadTaskWork: function (taskCode, ecCode, isCheckOut) {
                    ...
                    this.$root.$refs.task.operator = config.status.e_update;

                    if (ecCode) {
                        this.$root.$refs.caculation.operator = config.status.e_update;
                    } else {
                        this.$root.$refs.caculation.operator = config.status.e_new;
                    }
                    ...
                    if (this.$root.$refs.caculation.ecCode == ecCode) {
                        this.$root.$refs.caculation.$refs.editeccontent.ecCode = ecCode;
                    }
                    this.$root.$refs.caculation.ecCode = ecCode;
                    this.$root.$refs.caculation.refreshCode = ecCode;
                    this.$root.$refs.caculation.refreshIndex += 1;
                ...
                clearFilterName:function () {
                    this.$root.$refs.task.$refs.tasktool.items[0].filterName ='';
                    this.$root.$refs.caculation.$refs.ecmethods.filterName = '';
                    this.$root.$refs.caculation.$refs.caculationtools.data.items[0].filterName = '';
                },
                // 选中一个TASK
