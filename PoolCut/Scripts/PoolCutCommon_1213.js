...
function OpenUploadPage() {
    GSDialog.Open('导入数据', 'Pages/UploadImportData.html', null, function (result) {
        if (result) {
            window.location.reload();
        }
    }, 600, 400);
...
function OpenUploadTrustPage() {
    GSDialog.Open('导入打标资产', 'Pages/UploadImportTrustData.html', null, function (result) {
        if (result) {
            window.location.reload();
        }
    }, 600, 300);
...
function DownloadReport() {
    //下载'透视报表'
    var fileUrl = 'https://poolcutwcf/PoolCut/Files/Reports/DailyReport/资产池日统计报表.xlsm'//.format(Math.random(100, 1000000), Math.random(1, 1000000));
...
function GetCheckedPool() {
    ...
    return JSON.parse(htmlPoolHeader);
}
