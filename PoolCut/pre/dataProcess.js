...
var dataProcess = {
                        ...
                        switch (fieldType) {
                            ...
                            case "TargetLookup":
                                ...
                                if ($(this).attr("DataSourceType") == "Static") {
                                    ...
                                    console.log({ ECIndex: i, PIndex: rIndex, FIndex: fIndex, TargetType: currentParameter[0].TargetType });
                                    targetTypeGroup.push({ ECIndex: i, PIndex: rIndex, FIndex: fIndex, TargetType: currentParameter[0].TargetType });
                                    field = $.extend(field, { "SuggestedItems": SuggestedItems });
                                }
                                else {
                                    var parameterName = $(this).attr("ParameterName");
                                    ...
                                    var codeDictionaryCodes = '<items><item><code>AddressLookUp</code></item></items>';
    ...
    getPoolAmount: function (ParentPoolId, sql) {
        var sqlStr = "declare @targetPoolId bigint = " + ParentPoolId + "  " + sql;
        var serviceUrl = GlobalVariable.PoolCutServiceURL
            ...
            $.each(present.Field, function (j, field) {
                ...
                switch (field.FieldType) {
                    case "TextBox":
                        ...
                        var replaceValue = variableTemp.format(field.ParameterName, strType[0], field.Value);
    ...
    ecModelToXml: function (ecModel) {
        ...
        $(ecModel.XMLSqlQueryEC.Parameters).each(function (i, vp) {
            switch (vp.Type) {
                case "SqlParameter":
                    vp.Value = that.getDefaultParameters(vp, ecModel);
                ...
                case "ReplaceOperator":
                    ...
                    retParameter += paratemp.format(vp.Name, vp.Type, html2Escape(vp.Operator));
                    break;
                case "TargetLookup":
                    var itemcontex = that.getDefaultParameters(vp, ecModel);
        ...
        $(ecModel.XMLSqlQueryEC.Presentation).each(function (i, vs) {
            ...
            $(vs.Field).each(function (i, vf) {
                switch (vf.FieldType) {
                    case "ReplaceOperator":
                        ...
                        if (vf.SuggestedOperators.length > 0) {
                            ...
                            $(vf.SuggestedOperators).each(function (i, vo) {
                                ...
                                operatorItems += operator.format(vo.Name, html2Escape(vo.Value));
                            });
                            field = field.format(html2Escape(vf.FieldText), vf.ParameterName, vf.FieldType, operatorItems);
                        } else {
                    ...
                    case "ReplaceLookup":
                        if (vf.DataSourceType == "Static") {
                            ...
                            if (vf.SuggestedItems && vf.SuggestedItems.length > 0) {
                                ...
                                field = field.format(html2Escape(vf.FieldText), vf.ParameterName, vf.FieldType, vf.DataSourceType, suggestedItems);
                            } else {
                                field = field.format(html2Escape(vf.FieldText), vf.ParameterName, vf.FieldType, vf.DataSourceType, '<Item Name="" Value="" Excluded="" IsNew=""/>');
                            }
                            retRow += field;
                        }
                            ...
                            retRow += field.format(html2Escape(vf.FieldText), vf.ParameterName, vf.FieldType, vf.DataSourceType, (vf.LookupType == null ? "" : vf.LookupType), (vf.ConnectionStrings == null ? "" : vf.ConnectionStrings), (vf.Query == null ? "" : vf.Query));
                        }
                        break;
                    case "TargetLookup":
                        if (vf.DataSourceType == "Static") {
                            ...
                            if (vf.SuggestedItems && vf.SuggestedItems.length > 0) {
                                ...
                                field = field.format(html2Escape(vf.FieldText), vf.ParameterName, vf.FieldType, vf.DataSourceType, (vf.LookupType == null ? "" : vf.LookupType), suggestedItems);
                            } else {
                                field = field.format(html2Escape(vf.FieldText), vf.ParameterName, vf.FieldType, vf.DataSourceType, (vf.LookupType == null ? "" : vf.LookupType), '<Item Name="" Value="" Tolerance="" />');
                            }
                            retRow += field;
                        }
                        break;
                    case "CheckBox":
                        ...
                        retRow += field.format(html2Escape(vf.FieldText), vf.ParameterName, vf.FieldType);
                        break;
                    case "TextBox":
                        var field = '<Field FieldText="{0}" ParameterName="{1}" FieldType="{2}"/>';
                        retRow += field.format(html2Escape(vf.FieldText), vf.ParameterName, vf.FieldType);
                    ...
                    case "DQueryDisplay":
                        ...
                        retRow += field.format(html2Escape(vf.FieldText), vf.FieldType, vf.Format, vf.Dquery);
                        break;
                    default:
                        break;
                }
            ...
            retPresentation += t_Row.format(html2Escape(vs.RowText), retRow);
