...
var DataOperate = {
   ...
   /*获取DropDown的数据（同步）*/
    ...
    ViewgetSceneByCode: function (SceneCode, callback) {
        var sContent = "{'SPName':'getSceneByCode','Params':{" + "'SceneCode':'" + SceneCode + "'" + "}}";
        var serviceUrl = DataOperate.wizardService + "DataRead?dbName=" + DataOperate.dbName + "&schema=" + DataOperate.schema + "&json=" + sContent;
        console.info(serviceUrl)
        serviceUrl = encodeURI(serviceUrl);
        $.ajax({
    ...
    //返回所有资产明细
    //专项计划标识:TrustCode,资产编号:AccountNo,资产池:PoolDBName,客户编号:DealSum,交易时间：ropertyRedemptionDate,客户编号CustomerId,分行：DevisionName
    getSceneAccountMSG:function (callback) {
        var sContent = "{'SPName':'getSceneAccountMSG','Params':{}}";
    var serviceUrl = DataOperate.wizardService + "DataRead?dbName=" + DataOperate.dbName + "&schema=" + DataOperate.schema + "&json=" + sContent;
    console.info(serviceUrl)
    $.ajax({
        url: serviceUrl,
