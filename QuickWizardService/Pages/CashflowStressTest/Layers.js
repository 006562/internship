    ...
    'zh-CN': {
        Sections: [{
            ...
            Buttons: [
                ...
                { Text: '生成现金流模型', Click: 'GenerateCashflowModel()', Class: 'btn btn-primary' },
                { Text: '查看现金流模型', Click: 'ShowCashflowModel()', Class: 'btn btn-default' },
                { Text: '更新费用/偿付顺序', Click: 'UpdateCashflowModel()', Class: 'btn btn-default' },
                { Text: '保存为模板', Click: 'DataTemplate.SaveAsTemplate()', Class: 'btn btn-primary' },
                { Text: '应用模板', Click: 'DataTemplate.ChooseTemplate()', Class: 'btn btn-primary' }
            ]
        }]
    },
...
$(function () {
    ...
    $('#editPaymentSequenceBtn').click(function () {
        var url = '/TrustManagementService/TrustManagement/FeeSettings/PaymentSequenceSetting.html?tid=' + BusinessIdentifier;
        window.open(url, '_blank');
        //top.GSDialog.Open('&nbsp;', url, null, null, 1120, 550);
        //$.showModalDialog({
        //    title: '维护分层偿付顺序',
        //    url: url,
        //    height: 600,
    ...
    $('#editFeeSettingsBtn').click(function () {
        var url = '/TrustManagementService/TrustManagement/FeeSettings/feesettings.html?tid=' + BusinessIdentifier;
        //top.GSDialog.Open('费用信息', url, null, null, 800, 500);
        window.open(url, '_blank');
    });
...
function AddPPS() {
    var pps = { PlanPayDate: '', PlanAmount: '' };
    var obsPPS = ko.mapping.fromJS(pps);
    viewModel.Sections()[0].FieldsSetting.PaymentScheduleData.push(obsPPS);
    setFieldPlugins();
}
function RemovePPS(obj) {
    var index = $(obj).attr('itemIndex');
    var obsObj= viewModel.Sections()[0].FieldsSetting.PaymentScheduleData()[index];
    viewModel.Sections()[0].FieldsSetting.PaymentScheduleData.remove(obsObj);
}
...
function UpdateCashflowModel() {
	var tpi = new TaskProcessIndicatorHelper();
	tpi.AddVariableItem('TrustId', trustId, 'String');
	tpi.ShowIndicator('Task', 'GenerateCashflowTask', function () { });
...
var DataTemplate = {
    SaveAsTemplate: function () {
        top.GSDialog.Prompt('保存模板', '模板名称：', null, 'text', function (action, val) {
            if (action != 'ok') return;
            Loading.Show('正在保存模板...');
            var executeParam = { SPName: 'Template.usp_SaveTrustTemplate', SQLParams: [] };
    ...
    ChooseTemplate: function () {
        var dialogPage = '/QuickWizardService/Pages/CashflowStressTest/ViewTrustDateSet_Dialog.html?trustId=' + trustId + '&templateType=Trust';
        top.GSDialog.Open('选择模板', dialogPage, {}, function (val) {
            if (!val) { return; }
            Loading.Show('正在应用模板...');
            var executeParam = { SPName: 'Template.usp_ApplyTrustTemplate', SQLParams: [] };
            executeParam.SQLParams.push({ Name: 'TrustId', Value: trustId, DBType: 'int' });
            executeParam.SQLParams.push({ Name: 'TemplateID', Value: val, DBType: 'string' });
            var serviceUrl = GlobalVariable.QuickWizardServiceUrl + 'CommonGetWithConnName?connName=TrustTemplate&exeParams=' + encodeURIComponent(JSON.stringify(executeParam));
            CallWCFSvc(serviceUrl, true, 'GET', function (d) {
                Loading.Close();
                parent.qwFrame.ReloadStep('dateset');
                parent.qwFrame.ReloadStep('fee');
                parent.qwFrame.ReloadStep('sequence');
