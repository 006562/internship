        ...
        Sections: [{
            ...
            FieldsSetting: {
                ...
                Fields: [
                    ...
                    { ItemId: '5', ItemCode: 'MaxTerm', ItemAliasValue: '产品最长期数', DataType: 'int', IsCompulsory: 0, IsDisplay: 1, ItemValue: '' }
                ]
            },
            Buttons: [
                ...
                { Text: '查看静态池', Click: 'ViewResult()', Class: 'btn btn-default' },
				{ Text: '查看回收数据', Click: 'ViewRetrieveResult()', Class: 'btn btn-default' }
            ]
        }]
    },
        ...
        Sections: [{
            ...
            FieldsSetting: {
                ...
                Fields: [
                    ...
                    { ItemId: '4', ItemCode: 'AssetType', ItemAliasValue: 'Asset Type', DataType: 'CustSelect', IsCompulsory: 1, IsDisplay: 1, ItemValue: '' },
                    { ItemId: '5', ItemCode: 'MaxTerm', ItemAliasValue: 'Max Term', DataType: 'int', IsCompulsory: 1, IsDisplay: 1, ItemValue: '' }
                ]
            },
            Buttons: [
                { Text: 'Generate Static Pool Template', Click: 'GenerateTemplate("Section0Identity")', Class: 'btn btn-primary' },
                ...
                { Text: 'View static Pool Analysis Result', Click: 'ViewResult()', Class: 'btn btn-default' },
				{ Text: 'View static Pool Analysis Retrieve fund Result', Click: 'ViewRetrieveResult()', Class: 'btn btn-default' }
            ]
        }]
    },
...
function GenerateTemplate(sectionId) {
    ...
    tpi.AddVariableItem('maxterm', maxTerm, 'string');


    tpi.ShowIndicator('ConsumerLoan', 'GenerateStaticPoolTemplate', function (result) {
        //code in this place will be executed after the TaskIndicator be closed
    });
}

function Download() {
    window.location.href = 'https://poolcutwcf/QuickWizardService/Files/Output/StaticPoolTemplate.xls';
}
...
function UploadData(obj) {
    ...
    UploadFile(objId, fileName, 'StaticPool', function (d) {
        ...
        tpi.ShowIndicator('ConsumerLoan', 'LoadStaticPool_LoanFactory', function (result) {
            //code in this place will be executed after the TaskIndicator be closed
        });
    });
}
...
function ViewResult() {
    var orgCode = $('#OrganisationCode').val();
    var assetType = $('#AssetType').val();
    var pageUrl = 'ViewPoolData_tree.html?orgCode=' + orgCode + '&assetType=' + assetType;
    window.open(pageUrl);
}

function ViewRetrieveResult(){
	var orgCode=$('#OrganisationCode').val();
	var assetType=$('#AssetType').val();
	var pageUrl='ViewPoolData_RetrieveFund.html?orgCode='+orgCode+'&assetType='+assetType;
	window.open(pageUrl);
}
