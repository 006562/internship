...
function getLanguage(set) {
    var zh_CN = { Title01: "分层信息", Title02: "添加分层", Title03: "更新分层", Header: "操作", BtnSave: "保存信息", BtnAdd: "添加", BtnEdit: "编辑", BtnUpdate: "更新", BtnDelete: "删除", BtnClear: "清空", BtnRun: "运行", BtnLook: "查看结果" };
    var en_US = { Title01: "Layer Information", Title02: "Add Layer", Title03: "Update Layer", Header: "Operate", BtnSave: "Save Data", BtnAdd: "Add", BtnEdit: "Edit", BtnUpdate: "Update", BtnDelete: "Delete", BtnClear: "Clear", BtnRun: "Run", BtnLook: "Result" };
    switch (set) {
        case "zh-CN":
            return zh_CN;
        case "en-US":
...
ko.bindingHandlers.renderGridColumn = {
    init: function (element, valueAccessor) {
        ...
        html += '<input type="button" id="tb_View" class="btn btn-primary byn-sm" data-bind="attr: { dataIndex: $index }" onclick="trustBond_View(this)" value="' + language.BtnLook() + '"/> &nbsp;';
...
function trustBondAll_Run() {
    ...
    var taskCode = "QuickWizardProcess";
    ...
    $.ajax(
		{
		    ...
		    success: function (response) { CreateSessionCompleted(response); $.cookie('sessionid', response, { expires: 7 }); },
...
function lookItems() {
    ...
    if (sessionID !== undefined) {
        var _url = "https://poolcutwcf/TaskProcessServices/DataPublishService.svc/jsAccessEP/GetDataPublishContent?contextInfo={'DataRequest':'{0}','Params':{'SessionId':'{1}'}}";
        var FName = 'GETRESULT';
        var Type = 'RawValue';
        var str;
...
function trustBond_Run(obj) {
    ...
    editIndex = $(obj).attr('dataIndex');			//确定哪一行
    // var item = viewModel.GridView()[editIndex];
    // for (var key in item) {
    // //key就是ItemCode
    // var detail = viewModel.Detail();
    ...
    // sessionVariables_p=sessionVariables_p+ '<SessionVariable>'+
    // '<Name>'+key+'</Name>'+
    ...
    var sessionVariables_p = sessionVariables_p +
												'<SessionVariable>' +
													'<Name>editIndex</Name>' +
    ...
    sessionVariables_p = encodeURIComponent(sessionVariables_p);
    var taskCode = "RiskTransferBondRun";
    serviceUrl = serviceUrl + "?applicationDomain=" + indicatorAppDomain_p + "&sessionVariable=" + sessionVariables_p + "&taskCode=" + taskCode;
    jQuery.support.cors = true;
    //alert(sessionVariables_p);
