...
/// <reference path="asyncbox/asyncbox.js" />
...
var GSDialog = function () {
    ...
    var alert = function (aType, content, title, fnCallback) {
        ...
        switch (aType) {
            case 0:
                dispTitle = title || '成功';
                top.asyncbox.success(content, dispTitle, function (action) {
            ...
            case 1:
                dispTitle = title || '请确认';
                top.asyncbox.confirm(content, dispTitle, function (action) {
            ...
            case 3:
                ...
                top.asyncbox.error(content, dispTitle, function (action) {
            ...
            default:
                dispTitle = title || '警告';
                top.asyncbox.warning(content, dispTitle, function (action) {
    ...
    var prompt = function (title, tip, content, txtType, fnCallback) {
        //prompt 支持三种文本类型: text || textarea || password。
        //如需阻止窗口关闭，请在判断 action 值内加入 return false
        top.asyncbox.prompt(title, tip, content, txtType, fnCallback);
        // function (action, val) {
