...
(function ($) {
    mac.grid = function (self, cfg) {
        ...
        self.adjust = function (vp) {
            ...
            var g = pb ? 32 : 0;
            h = h - hd.height();
            bd.height(h - g -  (cfg.footerHeight || 0));
            self.adjustYBar();
            if (self.afterAdjust)
                self.afterAdjust();
        };
        ...
        self.load = function (pms) {
            ...
            var paramsBuilder = "";
            for (var key in ldr.params)
            {
                paramsBuilder += "'" + key + "':'" + ldr.params[key] + "',"
            }
            if (pb) {
                paramsBuilder += "'PageCount':0,'TotalCount':0";
            } else {
                paramsBuilder = paramsBuilder.substr(0, paramsBuilder.length - 1);
            }
            var sContent = "{'SPName':'" + ldr.spName + "','Params':{" + paramsBuilder + "}}";

            var serviceUrl = "https://"+ window.document.location.host +"/QuickWizardService/WizardService.svc/GetWizardData?appDomain=" + ldr.appDomain + "&json=" + sContent;
            if (pb)
            {
                serviceUrl = "https://" + window.document.location.host + "/QuickWizardService/WizardService.svc/GetWizardDataPaging?appDomain=" + ldr.appDomain + "&json=" + sContent;
            }
            $.ajax({
                url: serviceUrl,
                type: "GET",
               ...
               // async: (false),
                ...
                success: function (data) {
                    var ro = jQuery.parseJSON(data);
                    if (pb) {
                        if (ro.Success) {
                            var rolist = ro.Data;
                            self.loadData(rolist.List, rolist);
                            if (pb)
                                pb.update(rolist.TotalCount, rolist.PageSize, rolist.PageCount, rolist.PageNo)
                        }
                        hd.find('.sort').show();
                    }
                    else {
