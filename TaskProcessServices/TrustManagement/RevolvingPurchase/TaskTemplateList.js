    ...
    GetPeriodData(function (list) {
        if (list) {
            ...
            $.each(list, function (i, item) {
                var t = item.EndDate ? getStringDate(item.EndDate).dateFormat('yyyyMMdd') : '';//yyyy-MM-dd
...
function SaveAssetPayment(Id, index) {
    ...
    if (rowfile.length > 0 && rowfile.val().length > 0) {
        ...
        uploadAssetFile(args, fileData, saveData);
    }
    else
        saveData();
...
function SaveAssetPaymentData(executeParam) {
    ExecuteGetData(true, svcUrl, 'TrustManagement', executeParam, function (data) {
        if (data[0].Result == 1) { alert('保存成功'); bindAssetPaymentStatistics(); }
        else if (data[0].Result == 0) alert('保存失败');
        else if (data[0].Result == 2) alert('未找到该条数据');
    });
}

var uploadAssetFile = function (args, fileData, callback) {
    $.ajax({
        url: 'https://poolcutwcf/TaskProcessServices/DataProcessService.svc/jsAccessEP/UploadOriginatorFile?' + args,
        type: 'POST',
        data: fileData,
        cache: false,
        dataType: 'json',
