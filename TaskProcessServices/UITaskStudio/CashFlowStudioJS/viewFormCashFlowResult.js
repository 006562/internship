viewFormCashFlowResult = function (appDomain, sessionId, taskCode) {
    ...
    var currentChartTypeName = "现金流柱状图";
    var baseConfig = {
        subtitle: 'cashflow',
        ytitle: '数量',
        valueSuffix: ''
                ...
                series: {
                    events: {
                        legendItemClick: function () {
                            var oneSeries = $("#chart1").highcharts().series;
    ...
    var regisUiEvents = function () {
        loadPage();
        $("#studioTabs").tabs();
        $("#studioTabs li").bind("click", function () {
            if (sessionId != ""){
                if ($(this).index() == 1) {
                    if (IsFirstCard) {
        ...
        $(".radioChart").change(function () {
            if ($(this).val() == 1) {
                ...
                currentChartTypeName = "现金流柱状图";
            } else {
                currentChartType = "spline";
                currentChartTypeName = "现金流曲线图";
            }
            printChart();
        });
