...
; (function ($, window, document, undefined) {
    ...
    var defaults = {
        ...
        resetContainer: true,
        recordCount: 0////added options for sharepoint
    };
    ...
    var plugins = {
        ...
        source: {
            ...
            "default": function (sourceOptions) {
                var datagrid = this;
                var options = {
                    url: datagrid.settings.url, stamp: Math.random()
                };
                $.extend(options, sourceOptions);
        ...
        pager: {
            "default": function (page, lastpage, pagerOptions) {
                ...
                var options = {
                    container: "div",
                    attrContainer: { 'class': 'pager-container' },
                    attrUl: {},
                    ...
                    lastPage: false,
                    hideDisabled: true,
                    behavior: false  // { "sliding": { "pages": 1 } }  "sliding"
                };
                if (pagerOptions) {
                ...
                var pagerExtremes = function (disabled, gopage, label) {
                    ...
                    if (disabled) {
                        ...
                        var prevOrNext = (gopage > page) ? ' next' : ' prev';

                        element = $("<" + options.item + ">", {
                            data: { "page": gopage },
                            "class": pluginName + "-page" + prevOrNext
                        });
                    }
                    if (element) {
                        element.append(
                ...
                var pageSize = datagrid.settings.paramsDefault.paging;
                var recordCount = datagrid.settings.recordCount;
                var start = (page - 1) * pageSize + 1;
                var end = (page * pageSize > recordCount) ? recordCount : page * pageSize;
                container.append($('<span class="current">' + start + '-' + end + '</span>'));
                ...
                container.append($('<span class="total">共&nbsp;' + recordCount + '&nbsp;记录</span>'));
                return container;
            }
        }
    };
    ...
    Plugin.prototype = {
        ...
        renderTable: function (result) {
            ...
            self.settings.recordCount = total;
            ...
            table = $("<table></table>").attr('class', this.settings.attr);
            ...
            if (result.data && result.data.length > 0) {
                ...
                // no data
                var emptyTr = '<tr><td colspan="' + this.settings.col.length + '">'
                emptyTr += this.getNoData() + '</td></tr>';
                table.append(tbody.append($(emptyTr)));
            }

            // pager
            if ($.inArray("top", this.settings.pagerPosition) >= 0 && result.data && result.data.length > 0) {
            ...
            // pager
            if ($.inArray("bottom", this.settings.pagerPosition) >= 0 && result.data && result.data.length > 0) {
                this.renderPager(total);
            }
