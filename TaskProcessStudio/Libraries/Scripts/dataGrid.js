var dataGrid = {
    ...
    setVariableRowAndColumn: function (row) {
        var that = this;
        var rowCount = row > this.vminrow ? row + 1: this.vminrow;
    ...
    setArrayRowAndColumn: function (row, col) {
        var that = this;
        var colCount = col > this.amincol ? col +1: this.amincol;
        var rowCount = row > this.aminrow ? row +1: this.aminrow;
    ...
    {
        var offset = $("#divDataInputTable").offset(),
                x = pageX - offset.left,
                y = pageY - offset.top;
    ...
    processContextMenuClicked: function (obj)
    {
        ...
        switch (action)
        {
            ...
            case "paste":
                ...
                break;
            case "insert":
                if (sheetArea === GcSpread.Sheets.SheetArea.rowHeader) {
    ...
    bindcontextmenu: function () {
        var that = this;
        //绑定菜单
        $("#divDataInputTable").bind("contextmenu", function (e) {
            that.processSpreadContextMenu(e);
        });

        $("#divDataInputTable").on("click", function (event) {
            that.hideSpreadContextMenu();
        });
