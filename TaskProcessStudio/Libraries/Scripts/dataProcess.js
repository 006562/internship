var dataProcess = {
    ...
    taskXmlToJson: function(taskXml) {
        ...
        $(taskXml).find("Action").each(function(i) {
            actionModel[i] = { GroupName: '', ActionCode: $(this).attr('ActionCode'), ActionDisplayName: $(this).attr('ActionDisplayName'), FunctionName: $(this).attr('FunctionName'), SequenceNo: $(this).attr('SequenceNo'), IsCheck: false, Parameter: [] };
    ...
    ecXmlToJson: function(ecXml) {
        ...
        $(ecXml).find("main").each(function(i) {
            ecModel[i] = { IsCheck: false, Query: { Name: $(this).find("Query").attr("name"), Equation: $(this).find("Query").text() }, Parameters: [] };
    ...
    ecModelToXml: function(ecModel) {
        ...
        var content = '';
        var parameter, query;
        var queryTemp = "<Query name=\"{0}\">{{equation}}</Query><Presentation/>";
        ...
        $(ecModel).each(function() {
            ...
            query = queryTemp.format(queryModel.Name).replace("{{equation}}", queryModel.Equation.replace(new RegExp("&&", "g"), "&amp;&amp;").replace(new RegExp("<", "g"), "&lt;").replace(new RegExp(">", "g"), "&gt;"));
        ...
        $.ajax({
            ...
            success: function(functionXML) {
                $(functionXML).find("Functiongruop").each(function(key, value) {
                    ...
                    functionList.push({ Name: $(value).attr("Name"), list: items, isOpen: false })
                });
            }
        });
        return functionList;
    ...
    getRunTaskSessionVariables: function (variableModel, taskCode, ecCode) {
        ...
        var vVariableTemplate = "<SessionVariable><Name>{0}</Name><Value>{1}</Value><DataType>{2}</DataType><IsConstant>{3}</IsConstant><IsKey>{4}</IsKey><KeyIndex>{5}</KeyIndex></SessionVariable>";
        for (var i = 0; i < variableModel.length; i++) {
            strReturn += vVariableTemplate.format(variableModel[i].Name, variableModel[i].Value, variableModel[i].DataType, variableModel[i].IsConstant, variableModel[i].IsKey, variableModel[i].KeyIndex);
        }
    ...
    scriptForEC: function (appDomain, strTaskScript, taskCode, criteriaCode, response, scriptTemplates, callback) {
        ...
        if (response != undefined) {
            strTaskScript += scriptTemplates.ecScript.format(criteriaCode, response[0].XMLSqlQueryEC);
        }        
        var sContent = "{'SPName':'usp_GetProcessTask'," +
                          "'ProcessTaskCode':'" + taskCode + "'" +
                          "}";
    ...
    scriptForTask: function (appDomain, strTaskScript, taskCode, response, scriptTemplates, callback) {
        ...
        if (response != undefined) {
            ...
            strTaskScript += scriptTemplates.taskScript.format(taskCode, response[0].XMLProcessTask);
        }        
    ...
    scriptForSessionContext: function (appDomain, strTaskScript, taskCode, response, scriptTemplates, callback) {
        ...
        if (response != undefined) {
            $.each(response, function (i) {
                strTaskScript += scriptTemplates.processTaskContextScript.format(response[i].VariableName, response[i].VariableValue, response[i].VariableDataType, response[i].IsConstant, response[i].IsKey, response[i].KeyIndex);
            })
        }
        var sContent = "{'SPName':'usp_GetProcessTaskArrayByTaskCode'," +
                      ...
                      "}";
        webProxy.getQueryStoredProcedureProxy(appDomain, sContent, function (response) {
            self.scriptForSesseionContextArray(strTaskScript, response, scriptTemplates, callback);
        });
    },
    scriptForSesseionContextArray: function (strTaskScript, response, scriptTemplates, callback) {
        if (response != undefined) {
            $.each(response, function (i) {
                ...
                for (var p in response[i]) {
                    if (p != "ItemName") {
                        strTaskScript += scriptTemplates.processTaskArrayScript.format(itemName, response[i][p], j);
                        j += 1;
                    }
                }
            })
