...
var DataOperate = {
    ...
    savePageData: function (array, callback) {
        ...
        var json = "{'DBName':'" + DataOperate.dbName + "','Schema':'" + DataOperate.schema + "','SPName':'usp_SavePageItems','Params':{'Items':'" + xml + "'}}";
        json = "<SessionContext>{0}</SessionContext>".format(json);
        var serviceUrl = DataOperate.wizardService + "DataCUD";
        $.ajax({
            type: "POST",
            url: serviceUrl,
            dataType: "json",
   ...
   /*获取DropDown的数据（同步）*/
    ...
    getGuid: function (callback) {
        var serviceUrl = DataOperate.wizardService + "GetGuid";
        $.ajax({
            url: serviceUrl,
            type: "GET",
            contentType: "application/json; charset=utf-8",
    ...
    transationManagerData: function (callback) {
        var sContent = "{'SPName':'usp_GetTransferAssets','Params':{}}";
        var serviceUrl = DataOperate.wizardService + "DataRead?dbName=" + DataOperate.dbName + "&schema=" + DataOperate.schema + "&json=" + sContent;
    ...
    viewUpdateTransferProperty: function (BussinessNo, TransferMethod, PoolDBName, AccountNo, TransferDate, DealSum, TheorySum, FactSum, Dealer, StartDate, DealerOpponent, Remark) {
        ...
        var args = argTemplate.format(DataOperate.dbName, DataOperate.schema, BussinessNo, TransferMethod, PoolDBName, AccountNo, TransferDate, DealSum, TheorySum, FactSum, Dealer, StartDate, DealerOpponent, Remark);
    ...
    //交易管理（资产转让）编辑数据
    UpdateTransferProperty: function (BussinessNo, TransferMethod, PoolDBName, AccountNo, TransferDate, DealSum,
        Dealer, TheorySum, FactSum, StartDate, DealerOpponent, Remark, callback) {
    ...
    //会计场景(查看)   
    getSceneByCode: function (SceneCode, callback) {
        var sContent = "{'SPName':'usp_getSceneByCode','Params':{" + "'SceneCode':'" + subjectType + "'" + "}}";
        var serviceUrl = DataOperate.wizardService + "DataRead?dbName=" + DataOperate.dbName + "&schema=" + DataOperate.schema + "&json=" + sContent;
        console.info(serviceUrl)
        serviceUrl = encodeURI(serviceUrl);
        $.ajax({
    ...
    CreateFolder: function (businessId, parentNodeId, nodeName, callback)
    {
        ...
        var args = argTemplate.format(DataOperate.dbName,DataOperate.schema, businessId, parentNodeId, encodeURIComponent(nodeName));
    ...
    DocumentDelete:function(businessId, nodeId, isFolder)
    {
        ...
        var args = argTemplate.format(DataOperate.dbName,DataOperate.schema, businessId, nodeId, isFolder);
        var serviceUrl = DataOperate.wizardService + "DocumentDelete?" + args;
        $.ajax({
            url: serviceUrl,
            type: "POST",
