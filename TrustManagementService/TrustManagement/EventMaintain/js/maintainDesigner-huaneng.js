...
var getCalculateCodeList = function () {
    var executeParam = {
        SPName: 'usp_GetCalculationCodeList', SQLParams: [
        ]
    };
    ExecuteGetData(true, svcUrl, 'TrustManagement', executeParam, function (sourceData) {
        if (sourceData && sourceData.length > 0) {
...
var changEvent = function (obj) {
    ...
    if (obj) {
        ...
        $(obj).attr("style", "background-color: #56a3f5;color: #4d5765;");
...
//$("#selectedQualitativeList").mouseup(function () {
//    alert("hellow");
//});
$("#droppable").droppable({
...
$('.rightTi .nav a').click(function (event) {
    //alert($(this).attr('tab'));
    event.preventDefault();
    $(this).closest('li').siblings().find('a').removeClass('active');
    $(this).addClass('active');
    $('#' + $(this).attr('tab')).siblings().hide();
    $('#' + $(this).attr('tab')).show();
})
