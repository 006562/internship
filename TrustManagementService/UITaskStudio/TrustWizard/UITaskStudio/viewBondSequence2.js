...
viewBondSequence = function () {
    ...
    var viewTemplate = "<div class='divTab' style='width:99%;'>" +
                            ...
                            "<div style='width:775px'>    " +
                                ...
                                "<div class='grayDivTableHeadRight' style='width:140px'>同级分配<select id='basedOnType'><option value='ABasedOnCPB'>按剩余本金</option><option value='ABasedOnAVG'>平均分配</option><option value='ABasedOnManul'>手动分配</option></select></div>" +
                                "<div style='clear:both'></div>" +
                            "</div>" +
    ...
    var contentTemplate = "<div class='grayDivTableHeadCell' data='{0}' style='width:148px'>{0}</div>" +
                            ...
                            "<div class='grayDivTableCell' data='{0}' style='width:140px'><input type='checkbox' {5} /><input style='width:100px' type='text' value='{6}' />&nbsp;%" +
                                "&nbsp;</div>" +
                                "<div class='grayDivTableCellRight'  data='{0}' style='width:140px'><input style='width:100px' type='text' value='{7}' />&nbsp;%" +
                                "&nbsp;</div>"
    ;
    ...
    var selectChange = function () {
        ...
        if (itemCode != "") {
            //alert('ok');
            $("#AddNewScenario").html("<span>" + itemText + "</span>" + "<span class='removeScenario' style='float:right;'> X </span>");
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $.each(response, function (n, res) {
                    var start = new Date(eval(res.StartDate.replace("/Date(", "").replace(")/", ""))).toLocaleDateString();
                    var end = new Date(eval(res.EndDate.replace("/Date(", "").replace(")/", ""))).toLocaleDateString();
                    $("#PeriodStart").append($("<option />").val(start).text(start));
                    $("#PeriodEnd").append($("<option />").val(end).text(end));

                    startdates.push(new Date(eval(res.StartDate.replace("/Date(", "").replace(")/", ""))).toLocaleDateString());
                    enddates.push(new Date(eval(res.EndDate.replace("/Date(", "").replace(")/", ""))).toLocaleDateString());
                });               
            },
        ...
        $(function () {
            ...
            $("#" + name + " #bt_Save").click(function () {
                ...
                for (var i = 0; i < arrSequence.length; i++) {
                    ...
                    if (arrSequence[i] != "0") {
                        if (i >= arrSequence.length - 2)
                            arrCaculationSequence[i] = arrSequence[i] + "," + getClassName(i - arrSequence.length) + vIscheck;
                        else
                            arrCaculationSequence[i] = arrSequence[i] + "," + getClassName(i) + vIscheck;
                    }
                }
                ...
                if (!isupdated) {
                    ...
                    var newScenario = { "ScenarioName": scenarioName, "AllocationMethodCode": allocationMethodCode, "Periods": periods, "PresentationJson": currentJson };
